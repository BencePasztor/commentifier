...
interface PostHeaderProps extends Pick<Post, 'title' | 'description' | 'imageSource' | 'sourceUrl'> { }

const PostHeader = ({ title, description, imageSource, sourceUrl }: PostHeaderProps) => {
    return (
        <article className="relative flex items-center w-full p-4 isolate min-h-80">
            ...
            {/* Background Image */}
            <img className="absolute top-0 right-0 object-cover h-full -z-20" src={imageSource} alt={title} />
            {/* Post Content */}
            <div className="text-white max-w-prose">
                <h1 className="mb-4 text-3xl font-medium">{title}</h1>
                <p className="mb-4 text-sm">{description}</p>
                <SourceLinkButton className="text-xs" sourceUrl={sourceUrl}>Source</SourceLinkButton>
            </div>
        </article>
    )
}
